...
export default class CarrierShip extends Entity {
  ...
  dropItem() {
    const dropChance = Phaser.Math.Between(1, 3);
    const itemType = this.scene.player.lifes >= 5 ? this.items[Phaser.Math.Between(0, 1)] : this.items[Phaser.Math.Between(0, 2)];
    if (dropChance > 1) {
